...
import { useQuestionnaryMutations } from "../../../hooks/use-questionnary-mutations";
...
const QuestionnaireForm: React.FC = () => {
  ...
  const { createQuestionnary, createQuestionnaryIa } = useQuestionnaryMutations();
  ...
  const [useIA, setUseIA] = React.useState<boolean>(false);

  const onSubmit = async (data: QuestionnaireFormType) => {
    try {
      if (useIA) {
        const response = await createQuestionnaryIa.mutateAsync(data);
        navigate("/questionary/respond", { state: { questionnaire: response } });
      } else {
        const response = await createQuestionnary.mutateAsync(data);
        navigate("/questionary/respond", { state: { questionnaire: response } });
      }
    } catch (error) {
  ...
  if (createQuestionnary.isPending || createQuestionnaryIa.isPending) return <Loading />;
